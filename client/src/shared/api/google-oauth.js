...
export const fetchGoogleForm = async () => {
  const res = await axios.get(`${process.env.REACT_APP_API_ROOT}/api/account/google/signin`);
  return res.data;
}
...
export const fetchToken = async (redirect_code) => {
  try {
    const user = await axios.post(`${process.env.REACT_APP_API_ROOT}/api/account/google/redirect`, {
      code: redirect_code
    });

    const token = await axios.post(`${process.env.REACT_APP_API_ROOT}/api/account/signin`, user.data);
